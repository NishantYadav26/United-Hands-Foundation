    ...
    <div className="min-h-screen" style={{background: 'var(--bg-deep)'}}>
      ...
      <div className="pt-32 pb-24 px-4 sm:px-6">
        <div className="max-w-md mx-auto">
          <div className="glass-morph p-8 sm:p-10 rounded" data-testid="auth-form-container">
            ...
            {/* Toggle */}
            <div className="flex mb-8 rounded overflow-hidden" style={{background: 'var(--bg-deep)'}}>
              <button 
                onClick={() => setIsLogin(true)}
                className={`flex-1 py-3 text-sm font-semibold flex items-center justify-center gap-2 transition-colors ${isLogin ? '' : ''}`}
                style={{
                  background: isLogin ? 'var(--accent-teal)' : 'transparent',
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {!isLogin && (
                <>
                  ...
                  <input 
                    type="text"
                    placeholder="PAN Number (for 80G receipts)"
                    value={form.pan}
                    onChange={e => setForm({...form, pan: e.target.value.toUpperCase()})}
                    className="w-full rounded px-4 py-3 text-sm focus:outline-none"
                    style={{background: 'var(--bg-surface)', border: '1px solid var(--border-subtle)', color: 'var(--text-primary)'}}
                    data-testid="auth-pan-input"
                  />
                </>
              )}
